...
public class SortingTest {
    ...
    private static int[] DoHeapSort(int[] value) {
        ...
        // 자식 노드들을 가지고 있는 노드들에 대하여 heapify 하여 배열을 힙으로 만든다.
        for (int i = value.length / 2; i >= 0; i--) {
            heapify(value, i, value.length);
        }
        // 힙에서 원소 하나씩 제거하여 배열의 앞부터 넣는다
    ...
    private static void mergeSort(int[] value, int p, int r) {
        ...
        int q = (p + r) / 2;
        mergeSort(value, p, q); // 앞부분
        mergeSort(value, q + 1, r); // 뒷부분
        merge(value, p, q, r);
    }
...
//        System.out.println("Quicksort called");
        ...
        quickSort(value, p, lastPart1); // 앞부분
        quickSort(value, lastPart1 + 1, q); // 뒷부분
    }
